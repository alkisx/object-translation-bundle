...
final class TranslatableMappingManager
{
    ...
    public function translatableTypeFor(object $object): string
    {
        ...
        $type = (
            $class->getAttributes(Translatable::class)[0] ?? null
        )?->newInstance()->name ?? null;

        if (!$type) {
            throw new \LogicException(sprintf('The class "%s" is not translatable.', $object::class));
        }
    ...
    public function idFor(object $object): string
    {
        ...
        if (!$om) {
            throw new \LogicException(sprintf('No manager found for class "%s".', $object::class));
        }

        $id = $om->getClassMetadata($object::class)->getIdentifierValues($object);

        if (count($id) > 1) {
            throw new \LogicException(sprintf('The class "%s" has composite primary keys.', $object::class));
        }
    ...
    public function allTranslatableObjects(): iterable
    {
        foreach ($this->doctrine->getManagers() as $om) {
            foreach ($om->getMetadataFactory()->getAllMetadata() as $classMetadata) {
